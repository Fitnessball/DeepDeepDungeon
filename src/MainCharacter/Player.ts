...
export default class Player extends Phaser.Physics.Arcade.Sprite {
    ...
    constructor(scene: Phaser.Scene, x: number, y: number, texture: string, frame?: string | number) {
        ...
        sceneEvents.on('arrowUp', () => {
            this.touchState = TouchState.NONE
        });
        sceneEvents.on('arrowDownDown', () => {
            this.touchState = TouchState.DOWN
        });
        sceneEvents.on('arrowDownUp', () => {
            this.touchState = TouchState.UP
        });
        sceneEvents.on('arrowDownLeft', () => {
            this.touchState = TouchState.LEFT
        });
        sceneEvents.on('arrowDownRight', () => {
            this.touchState = TouchState.RIGHT
        });
        sceneEvents.on('ButtonCastSpell', () => {
            this.touchState = TouchState.ATTACK
        });
    }
    //SETSPELLS
    ...
    update(cursors: Phaser.Types.Input.Keyboard.CursorKeys) {
        ...
        if (!cursors) { return }
        if (Phaser.Input.Keyboard.JustDown(cursors.space) || this.touchState == TouchState.ATTACK) {
        ...
        if (this.playerState === PlayerState.IDLE) {
            ...
            if (cursors.left.isDown || this.touchState==TouchState.LEFT) {
            ...
            } else if (cursors.right?.isDown || this.touchState == TouchState.RIGHT) {
                this.setVelocity(vel, 0);
                this.anims.play('player-walk-right', true);
                this.setFlipX(false);
                ...
                this.activeNormalChest = undefined;
                this.activeSmallLightPillar = undefined;
            } else if (cursors.down?.isDown || this.touchState == TouchState.DOWN) {
                ...
                this.activeHearthChest = undefined;
                this.activeSmallLightPillar = undefined;
            } else if (cursors.up?.isDown || this.touchState == TouchState.UP) {
                this.setVelocity(0, -vel);
